...
namespace IT {
	...
	void ShowTable(IdTable& idtable, std::ostream& stream_out) {
		...
		for (i = 0, j = 0; i < idtable.size; i++, j++) {
			...
			case SF: stream_out << setw(18) <<left << "стандартная функция" << " | "; break;
			case OP:
			...
			else if (idtable.table[i].iddatatype == SGN && (idtable.table[i].idtype == V || idtable.table[i].idtype == L))
				stream_out<<"`"<<(idtable.table[i].value.vsgn.sgn)<<"`";
			else stream_out << "-";
			stream_out << endl;
		}
